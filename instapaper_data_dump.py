...
from helpers.utils import log_info, log_error, convert_html_to_markdown, calculate_hash
...
def instapaper_data_dump():
    ...
    if not all([consumer_key, consumer_secret, username, password]):
        log_error(log_path, "Instapaper API credentials (INSTAPAPER_CONSUMER_KEY, INSTAPAPER_CONSUMER_SECRET, INSTAPAPER_USERNAME, INSTAPAPER_PASSWORD) not found in .env file. Please set them.")
        print("Error: Instapaper API credentials not set in .env. See instapaper_data_dump.py for details.")
        return

    client = InstapaperAPIClient(consumer_key, consumer_secret)
    metadata_manager = MetadataManager({"data_directory": data_directory, "article_output_path": article_output_path})

    log_info(log_path, "Attempting to authenticate with Instapaper API...")
    if not client.authenticate(username, password):
        log_error(log_path, "Instapaper API authentication failed. Check your username, password, consumer key, and consumer secret.")
    ...
    while True:
        ...
        new_bookmarks = [b for b in bookmarks_chunk if b.get('bookmark_id') not in [x.get('bookmark_id') for x in all_bookmarks]]
        if not new_bookmarks:
            log_info(log_path, "No new bookmarks on this page. Ending fetch.")
            break

        all_bookmarks.extend(new_bookmarks)
        log_info(log_path, f"Fetched {len(new_bookmarks)} new bookmarks. Total fetched: {len(all_bookmarks)}.")
    ...
    for bookmark in all_bookmarks:
        ...
        if metadata_manager.exists(ContentType.INSTAPAPER, uid):
            log_info(log_path, f"Article '{title}' ({uid}) already processed. Skipping.")
            continue
        ...
        try:
            ...
            error_msg = f"Failed to fetch content for '{title}' from {original_url}: {e}"
    ...
    log_info(log_path, f"Instapaper data dump finished. Processed {processed_count} new articles.")
