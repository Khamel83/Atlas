...
def display_evaluation_file(file_path):
    ...
    try:
        ...
        if data.get('evaluations'):
            ...
            target_evaluator = st.sidebar.selectbox("Select Evaluation to Correct:", evaluator_ids)

            feedback_type = st.sidebar.selectbox("Feedback Type:", ["Rating", "Correction", "Comment"])
            
            payload = {}
            if feedback_type == "Rating":
                payload['rating'] = st.sidebar.select_slider("Rate Quality:", ["Good", "Okay", "Bad"])
            
            payload['comment'] = st.sidebar.text_area("Comment:", key=f"comment_{file_path}")
            ...
            if feedback_type == "Correction":
                ...
                for e in data['evaluations']:
                    if e['evaluator_id'] == target_evaluator:
                        original_text = e['result'].get('summary_text', json.dumps(e['result']))
                        break
                payload['corrected_text'] = st.sidebar.text_area("Corrected Text:", value=original_text, height=250, key=f"correction_{file_path}")
            ...
            if st.sidebar.button("Submit Feedback", key=f"submit_{file_path}"):
                try:
                    ...
                    if not source_file_path:
                        st.sidebar.error("Source file path not found in evaluation data. Cannot save feedback.")
                        return
...
if not evaluation_files:
    ...
    st.info("Run the ingestion pipeline (`python run.py`) to generate content and corresponding evaluation files.")
else:
