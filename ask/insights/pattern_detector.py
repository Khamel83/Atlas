class PatternDetector:
    ...
    def _enhance_with_trends(self, patterns):
        ...
        for tag in patterns['tag_frequencies']:
            ...
            if len(trend_data) >= 2:
                ...
                older_count = sum(item['count'] for item in trend_data[:-2]) if len(trend_data) > 2 else 1
                
                trend_direction = 'rising' if recent_count > older_count else 'falling' if recent_count < older_count else 'stable'
    ...
    def get_pattern_insights(self):
        ...
        insights = {
            ...
            'potential_issues': len([alert for alert in patterns['alerts'] if alert['severity'] == 'warning']),
            'tag_diversity': len(patterns['tag_frequencies']) / max(patterns['total_occurrences'], 1),
            'average_tags_per_item': patterns['total_occurrences'] / max(len(patterns['tag_frequencies']), 1)
        }
