class RecallEngine:
    ...
    def _get_review_data(self, metadata):
        ...
        """
        return {
            'last_reviewed': metadata.type_specific.get('last_reviewed') if metadata.type_specific else None,
            'review_count': metadata.type_specific.get('review_count', 0) if metadata.type_specific else 0,
            'success_rate': metadata.type_specific.get('review_success_rate', 1.0) if metadata.type_specific else 1.0,
            'difficulty_rating': metadata.type_specific.get('difficulty_rating', 3) if metadata.type_specific else 3  # 1-5 scale
        }
