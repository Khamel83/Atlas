...
def _log_notification(notification_type: str, level: str, message: str, 
...
def _write_failure_log(failure_type: str, item: str, error: Exception, 
                      details: Dict[str, Any] = None) -> None:
...
def _send_desktop_notification(level: str, message: str, title: str = "Atlas Capture") -> bool:
...
def notify_capture_failure(item: str, error: Exception, 
    ...
    try:
        ...
        _send_desktop_notification(NotificationLevel.ERROR, desktop_msg, "Atlas Capture Failed")
...
def notify_processing_failure(capture_id: str, error: Exception, 
                            details: Dict[str, Any] = None) -> None:
    """
    ...
    try:
        ...
        desktop_msg = f"{message}\nError: {error_msg[:100]}{'...' if len(error_msg) > 100 else ''}"
        _send_desktop_notification(NotificationLevel.WARNING, desktop_msg, "Atlas Processing Failed")
        ...
        logger.error(f"Notification system failed while reporting processing failure: {e}")
        logger.error(f"Original processing failure - Capture ID: {capture_id}, Error: {error}")

def notify_system_issue(issue_type: str, message: str, level: str = NotificationLevel.WARNING,
                       details: Dict[str, Any] = None) -> None:
    """
...
def notify_success(operation: str, message: str, details: Dict[str, Any] = None) -> None:
    """
...
def get_notification_statistics() -> Dict[str, Any]:
    ...
    try:
        ...
        for notification in notifications:
            ...
            if timestamp:
                if not stats['oldest_notification'] or timestamp < stats['oldest_notification']:
                    stats['oldest_notification'] = timestamp
                if not stats['newest_notification'] or timestamp > stats['newest_notification']:
        ...
        stats['recent_failures'] = len([
            ...
            if n.get('timestamp', '') > recent_cutoff and 
               n.get('level') in [NotificationLevel.ERROR, NotificationLevel.CRITICAL]
        ])
...
def test_notifications() -> None:
    ...
    _send_desktop_notification(NotificationLevel.INFO, "Desktop notification test", "Atlas Test")
    ...
    notify_system_issue("test", "This is a test system issue", NotificationLevel.WARNING)
...
if __name__ == "__main__":
    ...
    for notification in history:
        print(f"  {notification.get('timestamp', '')}: {notification.get('level', '')} - {notification.get('message', '')}") 