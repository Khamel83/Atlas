...
class CaptureResult:
    ...
    def __init__(self, success: bool, capture_id: str = None, error: str = None, 
...
def _log_capture_operation(capture_id: str, operation: str, success: bool, 
...
def capture_url(url: str, user_context: Dict[str, Any] = None) -> CaptureResult:
    """
...
def capture_file(file_path: str, user_context: Dict[str, Any] = None) -> CaptureResult:
    """
...
def capture_text(text: str, content_type: str = "text", 
                user_context: Dict[str, Any] = None) -> CaptureResult:
    """
...
def get_capture_status(capture_id: str) -> Dict[str, Any]:
    ...
    try:
        ...
        if not os.path.exists(metadata_path):
            return {
                'found': False,
                'error': f'Capture ID {capture_id} not found'
            }
    ...
    try:
        ...
        for filename in os.listdir(metadata_dir):
            ...
            try:
                ...
                timestamp = metadata.get('capture_timestamp')
                if timestamp:
                    if not stats['oldest_capture'] or timestamp < stats['oldest_capture']:
                        stats['oldest_capture'] = timestamp
                    if not stats['newest_capture'] or timestamp > stats['newest_capture']:
