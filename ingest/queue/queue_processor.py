...
from ingest.queue.processing_queue import ProcessingQueue, QueueItem, QueueStatus, Priority
...
class QueueProcessor:
    ...
    def _process_file_item(self, queue_item: QueueItem) -> Dict[str, Any]:
        ...
        try:
            ...
            if file_ext == '.opml':
                ...
                result = {"success": True, "message": f"Processed OPML file: {primary_path}"}
            elif file_ext == '.csv':
    ...
    def _process_generic_file(self, file_path: str, queue_item: QueueItem) -> Dict[str, Any]:
        ...
        try:
            ...
            output_path = os.path.join(output_dir, f"{queue_item.capture_id}_{filename}")
            ...
            metadata_path = os.path.join(output_dir, f"{queue_item.capture_id}_metadata.json")
    ...
    def _process_generic_text(self, text_content: str, queue_item: QueueItem) -> Dict[str, Any]:
        ...
        try:
            ...
            output_path = os.path.join(output_dir, f"{queue_item.capture_id}_processed.txt")
        ...
        try:
            ...
            if result.get("success", False):
                self.queue.mark_complete(
                    ...
                    result.get("result_paths", {})
                )
                self.processed_count += 1
                logger.info(f"Successfully processed {queue_item.capture_id} in {processing_time:.2f}s")
            else:
    ...
    def run_queue_processor(self, max_items: int = None, item_types: List[str] = None) -> Dict[str, Any]:
        """
        ...
        try:
            with ThreadPoolExecutor(max_workers=self.max_concurrent_processing) as executor:
                ...
                while self.running and items_submitted < max_items and not self.shutdown_event.is_set():
                ...
                for future in as_completed(futures, timeout=self.processor_timeout * 60):
                    try:
            ...
            logger.info(f"Queue processor completed: {self.processed_count} processed, {self.failed_count} failed")
            return stats
...
def run_queue_processor(max_items: int = None, item_types: List[str] = None) -> Dict[str, Any]:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--max-items", type=int, default=10, help="Maximum items to process")
