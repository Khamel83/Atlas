...
class ProcessingQueue:
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        self.queue_dir = self.config.get('PROCESSING_QUEUE_DIRECTORY', 'output/processing_queue')
    ...
    def add_to_queue(self, capture_id: str, item_type: str, source: str, 
                    user_context: Dict[str, Any] = None, priority: Priority = Priority.MEDIUM) -> bool:
        """
        ...
        try:
            with self._lock:
                ...
                if self._save_queue(items):
                    logger.info(f"Added {capture_id} to processing queue with priority {priority.name}")
                    return True
                else:
    ...
    def get_next_item(self, item_types: List[str] = None, processor_id: str = None) -> Optional[QueueItem]:
        """
        ...
        try:
            with self._lock:
                ...
                available_items.sort(key=lambda x: (x.priority.value, x.added_timestamp))
                ...
                # Save updated queue
                if self._save_queue(items):
                    logger.info(f"Assigned {next_item.capture_id} to processor {next_item.processor_id}")
                    return next_item
                else:
                    logger.error(f"Failed to save queue after assigning {next_item.capture_id}")
                    return None
            ...
            with self._lock:
                ...
                for item in items:
                    if item.capture_id == capture_id:
                        ...
                        if self._save_queue(items):
                            logger.info(f"Marked {capture_id} as processing by {processor_id}")
                            return True
                        else:
                            logger.error(f"Failed to save queue after marking {capture_id}")
                            return False
    ...
    def mark_complete(self, capture_id: str, result_paths: Dict[str, str] = None) -> bool:
        """
                ...
                for item in items:
                    if item.capture_id == capture_id:
                        ...
                        if self._save_queue(items):
                            ...
                            logger.error(f"Failed to save queue after completing {capture_id}")
                            return False
                ...
                for item in items:
                    if item.capture_id == capture_id:
                        ...
                        if item.processing_attempts < self.max_retry_attempts:
                            ...
                            delay = self._calculate_retry_delay(item.processing_attempts)
                            next_retry = datetime.now() + timedelta(seconds=delay)
                            item.next_retry_time = next_retry.isoformat()
                            
                            logger.info(f"Marked {capture_id} for retry in {delay} seconds")
                        else:
                            item.status = QueueStatus.FAILED
                            logger.warning(f"Marked {capture_id} as permanently failed after {item.processing_attempts} attempts")
                        
                        if self._save_queue(items):
                            return True
                        else:
                            logger.error(f"Failed to save queue after marking {capture_id} as failed")
                            return False
    ...
    def get_queue_status(self) -> Dict[str, Any]:
        ...
        try:
            with self._lock:
                ...
                now = datetime.now()
                for item in items:
                    if item.status == QueueStatus.PROCESSING and item.processing_started:
                        start_time = datetime.fromisoformat(item.processing_started)
                        processing_time = (now - start_time).total_seconds() / 60  # minutes
                ...
                for item in items:
                    if item.status == QueueStatus.PROCESSING and item.processing_started:
                        ...
                        if start_time < cutoff_time:
                            ...
                            item.status = QueueStatus.FAILED
                            item.last_error = f"Processing timed out after {max_age_hours} hours"
                ...
                for i, item in enumerate(items):
                    if item.capture_id == capture_id:
                        ...
                        if self._save_queue(items):
                            ...
                            logger.error(f"Failed to save queue after removing {capture_id}")
                            return False
...
def add_to_queue(capture_id: str, item_type: str, source: str, 
                user_context: Dict[str, Any] = None, priority: Priority = Priority.MEDIUM) -> bool:
...
def get_next_item(item_types: List[str] = None, processor_id: str = None) -> Optional[QueueItem]:
