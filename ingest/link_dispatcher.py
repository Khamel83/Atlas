...
def detect_url_type(url: str) -> str:
    ...
    if parsed.netloc and parsed.path:
        ...
        if (re.search(r'/\d{4}/\d{2}/', parsed.path) or  # Date pattern
            re.search(r'/[a-z0-9-]{10,}/', parsed.path) or  # Slug pattern
            '/article/' in parsed.path or
            '/post/' in parsed.path or
            '/story/' in parsed.path):
            return 'article'
...
def is_duplicate(url: str, config: Dict) -> bool:
    ...
    article_meta_path = os.path.join(config["article_output_path"], "metadata", f"{uid}.json")
    if os.path.exists(article_meta_path):
        return True
    
    # Check in YouTube metadata
    youtube_meta_path = os.path.join(config["youtube_output_path"], "metadata", f"{uid}.json")
    if os.path.exists(youtube_meta_path):
        return True
    
    # Check in podcast metadata (less likely but possible for direct episode URLs)
    podcast_meta_path = os.path.join(config["podcast_output_path"], "metadata", f"{uid}.json")
...
def dispatch_url(url: str, config: Dict) -> Tuple[bool, str]:
    ...
    """
    log_path = os.path.join(config.get("data_directory", "output"), "link_dispatcher.log")
    ...
    try:
        ...
        if url_type == 'youtube':
            ...
            log_info(log_path, f"Direct podcast URL handling not implemented yet: {url}")
            enqueue({
                "type": "podcast_url",
                "url": url,
...
def process_url_file(file_path: str, config: Dict) -> Dict[str, List[str]]:
    ...
    if not os.path.exists(file_path):
        log_error(os.path.join(config.get("data_directory", "output"), "link_dispatcher.log"), 
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        ...
        print("Usage: python -m ingest.link_dispatcher <url_or_file> or --instapaper <csv_file>") 