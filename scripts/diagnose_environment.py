...
class EnvironmentDiagnostic:
    ...
    def check_system_requirements(self):
        ...
        if py_version.major < 3 or (py_version.major == 3 and py_version.minor < 9):
            ...
            self.info.append(f"✅ Python {py_version.major}.{py_version.minor}.{py_version.micro} meets requirements")
                    ...
                    if errors:
                        for error in errors:
                            self.issues.append({
                                ...
                                "fix": error.fix_command or "See configuration documentation",
                                "critical": True
                            })
                    ...
                    if warnings:
                        for warning in warnings:
                            self.warnings.append({
                                ...
                                "fix": warning.fix_command or "Consider optimization"
                            })
    ...
    def generate_report(self, include_fixes=False):
        ...
        # Critical issues
        critical_issues = [issue for issue in self.issues if issue.get('critical', False)]
        if critical_issues:
        ...
        # Other issues
        other_issues = [issue for issue in self.issues if not issue.get('critical', False)]
        if other_issues:
...
def main():
    ...
    parser.add_argument("--test-apis", action="store_true",
                       help="Test connectivity to external APIs")
