...
def check_git_branch():
    ...
    try:
        ...
        if current_branch == 'main':
            ...
            console.print(f"[green]✔ On branch '[bold]{current_branch}[/bold]'. Good job![/green]")

    except (subprocess.CalledProcessError, FileNotFoundError):
        console.print("[yellow]Could not determine Git branch. Is this a Git repository?[/yellow]")
    console.print()
...
def check_directory_structure():
    ...
    console.print(table)
    if not all_ok:
        console.print("[yellow]Some directories are missing. The application might fail.[/yellow]")
    console.print()
...
def check_env_file():
    ...
    if not env_path_to_load:
        ...
        console.print(table)
        console.print(f"[yellow]Please create a '.env' file in the root or /config directory.[/yellow]")
        return
    ...
    table.add_row(".env file", f"[green]✔ Found at {env_path_to_load.relative_to(PROJECT_ROOT)}[/green]")
    ...
    for var, placeholder in ESSENTIAL_ENV_VARS.items():
        ...
        openai_key_is_openrouter = (os.getenv('OPENAI_API_KEY') or '').startswith('sk-or-v1-')

        if value:
            display_value = '********'
            table.add_row(f"  - {var}", f"[green]✔ Set[/green] (Value: {display_value})")
        
        elif var == 'OPENROUTER_API_KEY' and openai_key_is_openrouter:
             table.add_row(f"  - {var}", f"[grey70]○ Not Needed[/grey70] (Using OpenRouter key from OPENAI_API_KEY)")

        elif is_api_key and provider not in var.lower().replace('_api_key', ''):
             table.add_row(f"  - {var}", f"[grey70]○ Not Needed[/grey70] (Provider is {provider.capitalize()})")
        
        else:
            table.add_row(f"  - {var}", f"[yellow]✖ Not Set[/yellow] (placeholder: {placeholder})")
...
def check_dependencies():
    ...
    try:
        ...
        console.print(table)
        if not all_ok:
             console.print(f"[yellow]Some dependencies are missing. Please run 'pip install -r {REQ_FILE}'.[/yellow]")

    except ImportError:
        console.print("[red]Could not check dependencies: `importlib.metadata` not found. Are you on Python 3.8+?[/red]")
...
def scan_for_todos():
    ...
    try:
        ...
        if result.returncode == 0 and result.stdout:
            ...
            console.print(f"[yellow]rg command failed. Is ripgrep installed? Error: {result.stderr}[/yellow]")

    except FileNotFoundError:
        console.print("[yellow]ripgrep (rg) not found. Cannot scan for todos. Please install it for this feature.[/yellow]")
...
def check_error_logs():
    ...
    if not log_path.is_dir():
        console.print(f"[yellow]Log directory '{LOG_DIR}' not found. Skipping scan.[/yellow]")
        return
    ...
    for log_file in log_files[:5]: # Check 5 most recent logs
        ...
        if error_lines:
            ...
            table.add_row(log_file.name, Syntax(last_error, "log", theme="default", word_wrap=True))
    
    if error_found:
        console.print(table)
        console.print(f"[yellow]Errors found in recent logs. Check '{LOG_DIR}/' for details.[/yellow]")
    else:
        console.print("[green]✔ No 'ERROR' or 'CRITICAL' entries found in recent logs.[/green]")
    console.print()
...
def main():
    ...
    console.print("For a detailed project roadmap, see [bold]'docs/project_status_and_roadmap.md'[/bold].")
