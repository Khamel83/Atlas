...
class AtlasScheduler:
    ...
    def initialize_scheduler(self):
        ...
        jobstores = {
            'default': SQLAlchemyJobStore(url=f'sqlite:///{self.project_root}/scheduler.db')
        }
    ...
    def execute_job(self, job_id: str):
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                job.failure_count += 1
                self.logger.error(f"Job {job_id} failed with exit code {result.returncode}")
    ...
    def list_jobs(self):
        ...
        for job in self.jobs.values():
            ...
            last_run = "Never"
            if job.last_run:
                last_run = datetime.fromisoformat(job.last_run).strftime("%Y-%m-%d %H:%M")
        ...
        while True:
            ...
            if choice == "1":
                ...
                if not self.is_running():
                    console.print("[yellow]Starting scheduler in background...[/yellow]")
    ...
    def _add_custom_job(self):
        ...
        schedule = Prompt.ask("Schedule (cron format or interval like '30m', '1h', '1d')")
...
def main():
    ...
    if len(sys.argv) > 1:
        ...
        if arg == "--start":
            ...
            console.print("Available arguments: --start, --stop, --status, --list-jobs, --add-job, --interactive")
            sys.exit(1)
    else:
