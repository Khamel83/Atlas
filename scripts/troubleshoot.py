...
def run_auto_fix():
    ...
    if not config_path.exists():
        ...
        try:
            subprocess.run([
                sys.executable, "scripts/generate_env.py"
            ], text=True)
        except FileNotFoundError:
...
def generate_support_report():
    ...
    try:
        with open(report_file, 'w') as f:
            ...
            f.write(f"Platform: {subprocess.check_output(['uname', '-a'], text=True).strip()}\n\n")
...
def main():
    ...
    parser.add_argument("--auto-fix", action="store_true", 
                       help="Run automatic fixes without menu")
    parser.add_argument("--quick-check", action="store_true",
                       help="Run quick setup check only")
