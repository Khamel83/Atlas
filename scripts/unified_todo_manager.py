...
class UnifiedTodoManager:
    ...
    def setup_logging(self):
        ...
        logging.basicConfig(
            ...
            handlers=[
                logging.FileHandler(self.sync_log_file),
    ...
    def generate_todo_id(self, content: str, source: TodoSource, file_path: str = None) -> str:
    ...
    def add_todo(self, content: str, priority: TodoPriority = TodoPriority.MEDIUM,
                 source: TodoSource = TodoSource.MASTER, category: str = "general",
                 file_path: str = None, line_number: int = None,
                 estimated_time: int = None, dependencies: List[str] = None,
    ...
    def import_from_external_systems(self):
        ...
        console.print(f"[green]Imported {imported_count} TODOs from external systems[/green]")
    ...
    def import_from_inline_code(self) -> int:
        ...
        try:
            ...
            cmd = [
                'rg', '-i', r'(TODO|FIXME|XXX|HACK)', '-g', '*.py',
            ...
            if result.returncode == 0 and result.stdout:
                ...
                for line in lines:
                    ...
                    if len(parts) == 4:
                        ...
                        todo_id = self.generate_todo_id(comment, TodoSource.INLINE_CODE, file_path)
                    ...
                    for pattern in patterns:
                        ...
                        for match in matches:
                            if pattern.startswith('(?:'):  # Task ID pattern
                                ...
                                content_text = match.group(1) if match.groups() else match.group(0)
                                todo_id = self.generate_todo_id(content_text, TodoSource.DOCUMENTATION, doc_file)
                            
                            if todo_id not in self.todos:
                                priority = TodoPriority.MEDIUM
                                if any(word in content_text.upper() for word in ['CRITICAL', 'URGENT', 'MUST']):
                                    priority = TodoPriority.CRITICAL
                                elif any(word in content_text.upper() for word in ['HIGH', 'IMPORTANT']):
                                    priority = TodoPriority.HIGH
                                elif any(word in content_text.upper() for word in ['LOW', 'NICE']):
                                    priority = TodoPriority.LOW
                ...
                try:
                    ...
                    for pattern in patterns:
                        ...
                        for match in matches:
                            ...
                            todo_id = self.generate_todo_id(content_text, TodoSource.CHECKLIST, checklist_file)
    ...
    def display_unified_dashboard(self):
        ...
        console.print(Rule("[bold blue]Unified TODO Management Dashboard", style="blue"))
        
        total_todos = len(self.todos)
        if total_todos == 0:
            console.print("[yellow]No TODOs found. Run import to sync from external systems.[/yellow]")
            return
        ...
        for priority in [TodoPriority.CRITICAL, TodoPriority.HIGH, TodoPriority.MEDIUM, TodoPriority.LOW]:
            ...
            in_progress = len([t for t in priority_todos if t.status == TodoStatus.IN_PROGRESS])
            completed = len([t for t in priority_todos if t.status == TodoStatus.COMPLETED])
        ...
        # Next priority tasks
        pending_todos = [t for t in self.todos.values() if t.status == TodoStatus.PENDING]
        pending_todos.sort(key=lambda t: (
            0 if t.priority == TodoPriority.CRITICAL else
            1 if t.priority == TodoPriority.HIGH else
            2 if t.priority == TodoPriority.MEDIUM else 3
        ))
        ...
        # Critical warnings
        critical_todos = [t for t in self.todos.values() if t.priority == TodoPriority.CRITICAL and t.status == TodoStatus.PENDING]
        if critical_todos:
    ...
    def _add_todo_interactive(self):
        ...
        priority = Prompt.ask("Priority", choices=["critical", "high", "medium", "low"], default="medium")
        category = Prompt.ask("Category", default="general")
        source = Prompt.ask("Source", choices=["master", "dev_workflow", "documentation", "checklist"], default="master")
    ...
    def _update_todo_interactive(self):
        ...
        new_status = Prompt.ask("New status", choices=["pending", "in_progress", "completed", "cancelled", "blocked"], default=todo.status.value)
        new_priority = Prompt.ask("New priority", choices=["critical", "high", "medium", "low"], default=todo.priority.value)
    ...
    def _search_todos_interactive(self):
        ...
        results = []
        for todo in self.todos.values():
            if query.lower() in todo.content.lower() or query.lower() in todo.category.lower():
                results.append(todo)
    ...
    def _export_to_external_systems(self):
        ...
        dev_todos = [t for t in self.todos.values() if t.source == TodoSource.DEV_WORKFLOW or t.category == "foundation"]
...
def main():
    ...
    if len(sys.argv) > 1:
        ...
        if arg == "--import":
            ...
            console.print("Available arguments: --import, --dashboard, --sync, --export, --interactive")
            sys.exit(1)
    else:
