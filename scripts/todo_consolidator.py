...
class TodoConsolidator:
        ...
        for cursor_file in cursor_files:
            if cursor_file.exists():
                try:
                    ...
                    console.print(f"[yellow]Could not parse cursor file {cursor_file}: {e}[/yellow]")
                    ...
                    for pattern in patterns:
                        ...
                        for match in matches:
                            if pattern.startswith('(?:'):  # Task ID pattern
                                ...
                                task_id = f"DOC-{doc_file.replace('/', '_')}-{len(todos)+1}"
                ...
                try:
                    ...
                    for pattern in patterns:
                        ...
                        for i, match in enumerate(matches):
                            task_id = f"CHECKLIST-{checklist_file.replace('/', '_')}-{i+1}"
                    ...
                    console.print(f"[yellow]Could not scan {checklist_file}: {e}[/yellow]")
        ...
        with Progress(
            ...
            for source_name, scan_func in sources:
                ...
                try:
                    ...
                    progress.update(task, description=f"✓ {source_name} ({len(todos)} todos)")
                except Exception as e:
    ...
    def merge_and_deduplicate(self, all_todos: Dict[str, List[UnifiedTodo]]) -> Dict[str, UnifiedTodo]:
    ...
    def display_consolidated_report(self, merged_todos: Dict[str, UnifiedTodo]):
        ...
        console.print(Rule("[bold blue]Unified TODO Consolidation Report", style="blue"))
        ...
        for status, count in status_counts.items():
            ...
            summary_table.add_row(f"Status: {status.value.title()}", str(count), f"{percentage:.1f}%")
        ...
        for priority in [TodoPriority.CRITICAL, TodoPriority.HIGH, TodoPriority.MEDIUM, TodoPriority.LOW]:
            priority_todos = [t for t in merged_todos.values() if t.priority == priority]
        ...
        for source, count in source_counts.items():
            percentage = (count / total_todos) * 100
            source_table.add_row(source.value.replace('_', ' ').title(), str(count), f"{percentage:.1f}%")
        
        console.print(source_table)
        
        # Next priority tasks
        pending_todos = [t for t in merged_todos.values() if t.status == TodoStatus.PENDING]
        ...
        # Critical warnings
        critical_todos = [t for t in merged_todos.values() if t.priority == TodoPriority.CRITICAL and t.status == TodoStatus.PENDING]
        if critical_todos:
    ...
    def export_to_dev_workflow(self):
        ...
        console.print(f"[green]Exported {len(dev_tasks)} tasks to dev_tasks.json[/green]")
