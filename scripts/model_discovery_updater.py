...
class ModelDiscoveryUpdater:
    ...
    def discover_new_models(self) -> Dict[str, List[str]]:
        ...
        try:
            ...
            if not api_key:
                self.logger.warning("No OpenRouter API key found, using fallback discovery")
            ...
            if response.status_code != 200:
                self.logger.warning(f"OpenRouter API returned {response.status_code}, using fallback")
                return self._get_fallback_discovery()
            ...
            for model in models_data.get("data", []):
                ...
                is_free = (
                    ...
                    str(pricing.get("completion", "1")) == "0" and
                    str(pricing.get("request", "1")) == "0"
                )
                ...
                if is_free:
                    ...
                    if "/" in model_id:
                        ...
                        discovered_models["provider_breakdown"][provider].append(model_id)
            
            self.logger.info(f"Discovered {len(discovered_models['free_models'])} free models")
            self.logger.info(f"Found {len(discovered_models['new_free_models'])} new free models")
    ...
    def _get_fallback_discovery(self) -> Dict[str, List[str]]:
        ...
        return {
            ...
            "provider_breakdown": {
                ...
                "microsoft": ["microsoft/phi-3-mini-128k-instruct:free", "microsoft/phi-3-medium-128k-instruct:free"]
            }
        }
    
    def _is_fast_model(self, model_id: str, description: str, context_length: int) -> bool:
        """Determine if a model is optimized for speed"""
        # Check model name patterns
        fast_patterns = [
            "7b", "mini", "small", "fast", "turbo", "lite", "quick"
        ]
        ...
        # Check description
        if any(word in description for word in ["fast", "quick", "efficient", "lightweight"]):
            return True
    ...
    def _is_quality_model(self, model_id: str, description: str, context_length: int) -> bool:
        """Determine if a model is optimized for quality"""
        # Check model name patterns
        quality_patterns = [
            "70b", "34b", "13b", "large", "pro", "premium", "advanced", "r1", "v3"
        ]
        ...
        # Check description
        if any(word in description for word in ["advanced", "high-quality", "reasoning", "complex"]):
            return True
    ...
    def generate_usage_report(self) -> Dict:
        ...
        # Cost analysis
        total_cost = sum(stats.get("total_cost", 0) for stats in tracker.usage_data.get("models", {}).values())
        free_requests = sum(
            stats.get("total_requests", 0) 
            for model, stats in tracker.usage_data.get("models", {}).items()
            if ":free" in model
        )
        total_requests = sum(stats.get("total_requests", 0) for stats in tracker.usage_data.get("models", {}).values())
        
        report["cost_analysis"] = {
            "total_cost": total_cost,
            "free_usage_percentage": (free_requests / total_requests * 100) if total_requests > 0 else 0,
            "estimated_savings": total_cost * (free_requests / total_requests) if total_requests > 0 else 0
        }
        
        # Generate recommendations
        if report["cost_analysis"]["free_usage_percentage"] < 70:
            report["recommendations"].append("Consider increasing free model usage to reduce costs")
        
        if total_cost > 10:  # $10 threshold
            report["recommendations"].append("High API costs detected - review model selection strategy")
        
        return report
    
    def suggest_config_optimizations(self, available_models: List[str], classifications: Dict[str, List[str]]) -> Dict:
...
""".format(timestamp=datetime.now().strftime("%Y%m%d_%H%M%S"))
    ...
    def run_full_update(self):
        ...
        try:
            ...
            suggestions = self.suggest_config_optimizations(available_models, classifications)
            ...
            if suggestions.get("config_updates"):
                ...
                script_file = f"update_model_config_{datetime.now().strftime('%Y%m%d_%H%M%S')}.sh"
    ...
    def print_summary(self, report: Dict):
        ...
        print(f"💵 Estimated Savings: ${usage['cost_analysis']['estimated_savings']:.4f}")
    ...
    def run_monthly_update(self):
        ...
        try:
            ...
            self.model_selector.discovery.cache["last_updated"] = datetime.now().isoformat()
            ...
            recommendations = self.generate_recommendations(discovered_models, usage_report)
            
            # Step 6: Create comprehensive report
            monthly_report = self.create_monthly_report(discovered_models, usage_report, recommendations)
    ...
    def generate_recommendations(self, discovered_models: Dict[str, List[str]], usage_report: Dict) -> Dict:
    ...
    def create_monthly_report(self, discovered_models: Dict[str, List[str]], usage_report: Dict, recommendations: Dict) -> Dict:
        ...
        return {
            ...
            "discovery_summary": {
                ...
                "provider_breakdown": {
                    provider: len(models) 
                    for provider, models in discovered_models["provider_breakdown"].items()
                }
            },
    ...
    def save_monthly_report(self, report: Dict):
        ...
        report_file = report_dir / f"model_discovery_report_{datetime.now().strftime('%Y%m')}.json"
        ...
        try:
            ...
            summary_file = report_dir / f"model_discovery_summary_{datetime.now().strftime('%Y%m')}.md"
...
"""
        ...
        for provider, count in report['discovery_summary']['provider_breakdown'].items():
