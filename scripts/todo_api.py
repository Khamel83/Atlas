...
def main():
    ...
    add_parser.add_argument('--source', choices=['master', 'dev_workflow', 'documentation', 'checklist'], default='master')
    ...
    status_parser.add_argument('new_status', choices=['pending', 'in_progress', 'completed', 'cancelled', 'blocked'])
    ...
    list_parser.add_argument('--status', choices=['pending', 'in_progress', 'completed', 'cancelled', 'blocked'])
    ...
    sync_parser.add_argument('--import', action='store_true', dest='import_flag', help='Import from external systems')
    sync_parser.add_argument('--export', action='store_true', dest='export_flag', help='Export to external systems')
    ...
    if args.command == 'add':
        ...
        if todos:
            ...
            for todo in todos:
                print(f"  {todo.id}: {todo.content} [{todo.status.value}] [{todo.priority.value}]")
        else:
        ...
        for todo in manager.todos.values():
            if args.query.lower() in todo.content.lower() or args.query.lower() in todo.category.lower():
                results.append(todo)
