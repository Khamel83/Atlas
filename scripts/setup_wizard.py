...
class AtlasSetupWizard:
    ...
    def run_setup(self, automated: bool = False, skip_deps: bool = False, config_only: bool = False) -> bool:
        """
            ...
            if result.returncode == 0:
                ...
                if not automated:
                    ...
                    if choice == "1":
                        ...
                        self.setup_state["warnings"].append("Dependency validation failed")
                        self._mark_step_complete(2)
                        return True
    ...
    def _step_directory_setup(self, automated: bool) -> bool:
        ...
        required_dirs = [
            "output", "output/articles", "output/articles/html", 
            "output/articles/markdown", "output/articles/metadata",
            "output/youtube", "output/podcasts", "config", "logs", "retries"
        ]
        ...
        if failed_dirs:
            ...
            if not automated:
                continue_anyway = self._get_yes_no(
                    "Continue setup despite directory creation failures", 
                    default=False
                )
    ...
    def _step_initial_test(self) -> bool:
        ...
        try:
            ...
            continue_anyway = self._get_yes_no(
                "Continue setup despite test failures", 
                default=True
            )
    ...
    def _step_setup_completion(self):
        ...
        print("   - Review configuration: python -c \"from helpers.config import load_config; print(load_config())\"")
        print()
...
Examples:
    ...
    parser.add_argument(
        "--config-only", 
        action="store_true",
        help="Only run configuration setup"
    )
