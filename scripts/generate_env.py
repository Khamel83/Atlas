...
class EnvGenerator:
    ...
    def generate_env_file(self, force: bool = False, interactive: bool = False, quiet: bool = False) -> bool:
        """
        ...
        try:
            ...
            if self.env_path.exists() and not force:
                ...
                response = input(f".env file already exists at {self.env_path}. Overwrite? (y/n): ")
                if response.lower() != 'y':
            ...
            existing_values = self._load_existing_env() if self.env_path.exists() else {}
            ...
            for key, schema in self.config_schema.items():
                try:
                    value = self._get_config_value(
                        key, schema, existing_values.get(key), 
                        interactive, quiet
                    )
            ...
            if not quiet:
                ...
                print(f"🔍 Validation: {'✓ Passed' if validation_result else '⚠ Issues detected'}")
    ...
    def _get_config_value(self, key: str, schema: Dict[str, Any], 
                         existing_value: Optional[str], interactive: bool, 
                         quiet: bool) -> str:
        ...
        if interactive or schema.get("required", False):
            ...
            if existing_value:
                use_existing = input(f"  {key} (current: {existing_value}): Keep existing? (y/n, default=y): ")
    ...
    def _prompt_for_value(self, key: str, schema: Dict[str, Any], 
                         existing_value: Optional[str]) -> str:
    ...
    def _validate_generated_env(self) -> bool:
        ...
        try:
            ...
            if model and "/" not in model:
                issues.append("Model name should typically contain provider/model format")
    ...
    def _show_next_steps(self):
        ...
        print(f"  2. Test configuration: python -c \"from helpers.config import load_config; print('✓ Config loads successfully')\"")
    ...
    def _validate_api_key(self, value: str) -> Tuple[bool, str]:
        ...
        if not any(value.startswith(prefix) for prefix in ['sk-', 'test-', 'api_']):
            return False, "API key format may be invalid (should start with sk-, test-, or api_)"
