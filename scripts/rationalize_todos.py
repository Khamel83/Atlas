...
class TodoRationalizer:
    ...
    def validate_critical_todos(self):
        ...
        critical_todos = {
            todo_id: todo for todo_id, todo in self.todos.items()
        ...
        for todo_id, todo in critical_todos.items():
            ...
            if len(content) < 10 or content.startswith("**") or content in ["CRITICAL", "HIGH", "MEDIUM", "LOW"]:
                console.print(f"[red]Critical TODO needs better description: {todo_id}[/red]")
                console.print(f"Current content: '{content}'")
                
                # Try to provide better description based on ID
                if "API" in todo_id:
                    todo["content"] = "Fix critical API consistency issues preventing proper system operation"
                elif "CAPTURE" in todo_id:
                    todo["content"] = "Implement critical capture system component to prevent data loss"
                elif "FOUNDATION" in todo_id:
                    todo["content"] = "Complete critical foundation task required for system stability"
                else:
                    todo["content"] = f"Complete critical task {todo_id} required for system operation"
    ...
    def print_summary(self):
        ...
        table.add_row("Final TODOs", str(self.stats["final_count"]), 
...
def main():
    ...
    if success:
        ...
        console.print("Run 'python3 scripts/unified_todo_manager.py --dashboard' to see the cleaned results.")
    else:
