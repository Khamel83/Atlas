...
def test_full_ingestion_pipeline(dummy_config):
    ...
    with patch("helpers.instapaper_ingestor.sync_playwright") as mock_playwright, patch(
        ...
        mock_page.locator.return_value.all.return_value = [] # Mock all() to return empty list
        mock_page.locator.return_value.inner_text.return_value = "Test Article"
        mock_page.locator.return_value.get_attribute.return_value = "https://example.com/article"
        mock_page.locator.return_value.inner_html.return_value = "<html><body>Test Content</body></html>"
