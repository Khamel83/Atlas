...
def test_full_ingestion_pipeline(dummy_config):
    ...
    with patch("helpers.article_fetcher.fetch_and_save_article", return_value=True) as mock_article:
    ...
    with patch.object(podcast_ingestor, "fetch_content", return_value=(True, [{"title": "Test Podcast", "guid": "g", "enclosures": [MagicMock(href="http://audio.mp3")], "links": []}])), \
         patch.object(podcast_ingestor, "process_content", return_value=True) as mock_process:
    ...
    with patch("helpers.youtube_ingestor.extract_video_id", return_value="vidid"), \
        ...
        result = youtube_ingestor.ingest_single_video("https://youtube.com/watch?v=vidid")
        assert result is True
    ...
    with patch("helpers.instapaper_ingestor.sync_playwright") as mock_playwright, \
         patch("helpers.instapaper_ingestor.log_info"), \
         patch("helpers.instapaper_ingestor.log_error"):
        mock_playwright.return_value.__enter__.return_value.chromium.launch.return_value.is_connected.return_value = True
