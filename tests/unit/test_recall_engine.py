...
class FakeContentMetadata:
    def __init__(self, title, updated_at, last_reviewed=None, review_count=0):
        ...
        self.type_specific = {'last_reviewed': last_reviewed, 'review_count': review_count} if last_reviewed else {}
...
def fake_metadata_manager():
    ...
    items = [
        FakeContentMetadata("A", (now - timedelta(days=20)).isoformat(), (now - timedelta(days=10)).isoformat(), 0),
        FakeContentMetadata("B", (now - timedelta(days=10)).isoformat(), (now - timedelta(days=1)).isoformat(), 1),
...
def test_mark_reviewed(fake_metadata_manager):
    engine = RecallEngine(fake_metadata_manager)
    item = FakeContentMetadata("Test", (datetime.now() - timedelta(days=50)).isoformat(), None, 0)
