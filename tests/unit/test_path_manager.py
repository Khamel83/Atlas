...
def test_get_single_path(path_manager, tmp_path):
    ...
    assert (
        path_manager.get_single_path(ContentType.ARTICLE, "test_uid", PathType.AUDIO)
        is None
    )
...
def test_validate_path(path_manager, tmp_path):
    ...
    relative_valid = os.path.join(
        str(tmp_path), "articles", "..", "articles", "test.json"
    )
...
def test_path_set_get_path():
    ...
    path_set = PathSet(
        uid="test_uid", content_type=ContentType.ARTICLE, base_dir="/test", paths=paths
    )
...
def test_path_manager_with_custom_config(tmp_path):
    ...
    assert manager.get_base_directory(ContentType.ARTICLE) == os.path.join(
        str(tmp_path), "custom_articles"
    )
    assert manager.get_base_directory(ContentType.PODCAST) == os.path.join(
        str(tmp_path), "custom_podcasts"
    )
    assert manager.get_base_directory(ContentType.YOUTUBE) == os.path.join(
        str(tmp_path), "custom_youtube"
    )
...
def test_migrate_paths_failure(path_manager, tmp_path):
    ...
    with patch("shutil.copytree", side_effect=OSError("Permission denied")):
        assert (
            path_manager.migrate_paths(old_dir, new_dir, ContentType.ARTICLE) is False
        )
