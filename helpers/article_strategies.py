...
class FetchResult:
    ...
    def __init__(self, success: bool, content: str = None, method: str = None, 
                 error: str = None, is_truncated: bool = False, metadata: Dict[str, Any] = None,
                 title: str = None, strategy: str = None):
    ...
    def fetch(self, url: str, log_path: str = "") -> FetchResult:
        try:
            ...
            return FetchResult(
                ...
                content=response.text,
                method="12ft_bypass",
                metadata={"bypass_url": bypass_url, "status_code": response.status_code}
            )
        except requests.exceptions.RequestException as e:
...
class ArchiveTodayStrategy(ArticleFetchStrategy):
    ...
    def fetch(self, url: str, log_path: str = "") -> FetchResult:
        try:
            ...
            raise Exception(f"Archive creation failed with status {response.status_code}")
    ...
    def fetch(self, url: str, log_path: str = "") -> FetchResult:
        try:
            ...
            return FetchResult(
                ...
                metadata={"snapshot_url": snapshot_url, "timestamp": data["archived_snapshots"]["closest"]["timestamp"]}
            )
        except Exception as e:
    ...
    def fetch_with_fallbacks(self, url: str, log_path: str) -> FetchResult:
        ...
        for strategy in self.strategies:
            ...
            if result.success:
                ...
                if is_truncated:
                    log_info(log_path, f"{strategy.get_strategy_name()} succeeded but content appears truncated/low quality, trying next strategy...")
                    result.is_truncated = True
                    last_result = result
                    continue
                
                log_info(log_path, f"Successfully fetched {url} using {strategy.get_strategy_name()}")
                return result
            
            last_result = result
        
        # All strategies failed or returned low-quality content
        log_error(log_path, f"All fetch strategies failed or returned low-quality content for {url}")
        return last_result or FetchResult(success=False, error="All strategies failed or returned low-quality content") 