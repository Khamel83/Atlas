    ...
    try:
        ...
        metadata = {
            ...
            "fetch_details": {
                ...
                "attempts": (
                    result.metadata.get("attempts", []) if result.metadata else []
                ),
                "total_attempts": (
                    len(result.metadata.get("attempts", [])) if result.metadata else 1
                ),
                "is_truncated": result.is_truncated,
        ...
        try:
            ...
            log_error(
                log_path, f"Failed to update evaluation file for {url}: {eval_error}"
            )
...
def fetch_and_save_articles(config: dict) -> dict:
    ...
    results = {"success": success_count, "failed": failed_count, "total": len(urls)}

    log_info(
        "",
        f"Article processing complete: {success_count} success, {failed_count} failed",
    )
    return results
