...
from helpers.utils import generate_markdown_summary, log_error, log_info, calculate_hash
from process.evaluate import summarize_text, extract_entities, classify_content
...
PAYWALL_PHRASES = [
    ...
    "unlock this story", "your free articles", "to continue reading",
    "subscribe now", "subscription required", "premium content", 
    "members only", "register to continue", "paid subscribers only",
    "subscribe for full access", "subscribe for unlimited access",
    "login to read more", "create an account to continue",
    "please enable js", "please enable javascript", "disable any ad blocker",
    "javascript is disabled", "javascript required", "enable javascript",
    "this site requires javascript", "javascript must be enabled"
]
PAYWALL_ELEMENTS = [
    ".paywall", ".subscription-required", ".premium-content", 
    ".register-wall", ".subscription-wall", ".paid-content",
    "#paywall", "#subscribe-overlay", "#subscription-overlay",
    "div[data-paywall]", "[data-require-auth]"
]
MIN_WORD_COUNT = 150
...
def is_truncated(html_content: str, log_path: str) -> bool:
    ...
    if title_tag and title_tag.text:
        ...
        if content_length > 0:
            ...
            if ratio > TITLE_CONTENT_RATIO_THRESHOLD:
                log_info(log_path, f"Suspicious title-to-content ratio: {ratio:.2f}. Likely truncated.")
                return True
    ...
    for form in form_tags[:3]:  # Check only the first few forms
        ...
        if any(word in form_text for word in ['login', 'sign in', 'subscribe', 'register']):
            log_info(log_path, "Login/subscription form detected near the top of the page.")
            return True
    ...
    try:
        ...
        if word_count < MIN_WORD_COUNT:
            log_info(log_path, f"Content is very short ({word_count} words). Likely a teaser.")
            return True
    except Exception:
        # If readability fails, we can fall back to the raw text word count
        word_count = len(text.split())
        if word_count < MIN_WORD_COUNT:
            log_info(log_path, f"Content is very short ({word_count} words, raw). Likely a teaser.")
            return True
...
def _fetch_from_wayback_machine(url: str, log_path: str) -> str:
    ...
    try:
        ...
        log_error(log_path, f"An unexpected error occurred during Wayback Machine fetch for {url}: {e}")
        return None
...
def _fetch_from_archive_today(url: str, log_path: str) -> str:
    ...
    try:
        ...
        if archive_links:
            ...
            log_info(log_path, f"Found existing archive at {archive_url}. Fetching content.")
            ...
            submit_response = requests.post(submit_url, data=data, headers=headers, timeout=60)
            ...
            if submit_response.status_code == 200 and "Submitting" in submit_response.text:
                log_info(log_path, "Archive request submitted. Waiting for processing...")
                
                # Extract the job ID if available
                job_match = re.search(r'id="DIVALREADY".*?href="([^"]+)"', submit_response.text)
                if job_match:
                    ...
                    log_info(log_path, f"Successfully created and fetched new archive for {url}.")
                    return archive_response.text
        ...
        log_error(log_path, f"An unexpected error occurred during archive.today fetch for {url}: {e}")
        return None
        ...
        try:
            ...
            if classification_result:
                eval_file.add_evaluation(
                    ...
                    result={"classification": classification_result, "model": config.get("llm_model")}
                )
            ...
            log_error(log_path, f"Failed to create or update evaluation file for {md_path}: {e}")
...
def fetch_and_save_articles(config: dict):
    ...
    for url in urls:
        ...
        meta["fetch_details"]["fetch_time"] = (datetime.now() - fetch_start_time).total_seconds()
        ...
        try:
            ...
            try:
                ...
                eval_file.add_evaluation(
                    evaluator_id="ingestion_v1",
                    eval_type="ingestion_check",
                    result={"status": "success", "notes": f"Content fetched via {meta.get('fetch_method', 'unknown')}"}
                )
                ...
                if classification_result:
                    ...
                    meta["tags"].extend(classification_result.get("tier_1_categories", []))
                    meta["tags"].extend(classification_result.get("tier_2_sub_tags", []))
                    meta["category_version"] = "v1.0"
                    meta["last_tagged_at"] = datetime.now().isoformat()
                    meta["source_hash"] = calculate_hash(md_path)
                ...
                log_info(log_path, "Running standard summary/entity pipeline for article.")
