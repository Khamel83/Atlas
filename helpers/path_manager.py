...
class PathManager:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.type_directories = {
            ContentType.ARTICLE: config.get("article_output_path", os.path.join(self.data_directory, "articles")),
            ContentType.PODCAST: config.get("podcast_output_path", os.path.join(self.data_directory, "podcasts")),
            ContentType.YOUTUBE: config.get("youtube_output_path", os.path.join(self.data_directory, "youtube")),
            ContentType.INSTAPAPER: config.get("article_output_path", os.path.join(self.data_directory, "articles"))
        }
    ...
    def get_single_path(self, content_type: ContentType, uid: str, path_type: PathType) -> Optional[str]:
    ...
    def get_temp_path(self, content_type: ContentType, uid: str, suffix: str = "") -> str:
    ...
    def migrate_paths(self, old_base_dir: str, new_base_dir: str, content_type: ContentType) -> bool:
    ...
    def get_backup_path(self, content_type: ContentType, uid: str, timestamp: str = None) -> str:
...
def get_content_paths(content_type: ContentType, uid: str, config: Dict[str, Any]) -> PathSet:
    """Get paths for a content item."""
    manager = create_path_manager(config)
    return manager.get_path_set(content_type, uid)


def ensure_content_directories(content_type: ContentType, config: Dict[str, Any]) -> bool:
