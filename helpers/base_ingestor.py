...
from helpers.metadata_manager import (
    ContentType, ProcessingStatus, MetadataManager, 
    ContentMetadata, create_metadata_manager
)
from helpers.path_manager import PathManager, PathType, create_path_manager
from helpers.error_handler import (
    AtlasErrorHandler, ErrorCategory, ErrorSeverity, ErrorContext,
...
class IngestorResult:
    ...
    def __init__(self, success: bool, metadata: ContentMetadata = None, 
                 error: str = None, should_retry: bool = False):
...
class BaseIngestor(ABC):
    ...
    @abstractmethod
    def fetch_content(self, source: str, metadata: ContentMetadata) -> Tuple[bool, Optional[str]]:
        """
    ...
    def create_metadata(self, source: str, title: str = None, **kwargs) -> ContentMetadata:
    ...
    def handle_error(self, error_message: str, source: str = None, 
                    should_retry: bool = False, severity: ErrorSeverity = ErrorSeverity.MEDIUM) -> bool:
    ...
    def run_evaluations(self, content: str, metadata: ContentMetadata) -> bool:
        ...
        try:
            ...
            eval_file = EvaluationFile(source_file_path=markdown_path, config=self.config)
    ...
    def ingest_single(self, source: str, title: str = None, **kwargs) -> IngestorResult:
        ...
        try:
            ...
            if not success:
                ...
                return IngestorResult(success=False, metadata=metadata, 
                                    error=content_or_error, should_retry=True)
    ...
    def ingest_batch(self, sources: List[str], **kwargs) -> Dict[str, IngestorResult]:
        ...
        log_info(self.log_path, f"Batch ingestion complete: {successful} successful, {failed} failed")
...
class MediaBasedIngestor(BaseIngestor):
    ...
    def save_media_file(self, media_content: bytes, metadata: ContentMetadata, 
    ...
    def run_content_specific_evaluations(self, content: str, eval_file: EvaluationFile):
        ...
        classification_result = classify_content(content, self.config)
        content_type_detected = classification_result.get("content_type", "") if classification_result else ""
        
        if "interview" in content_type_detected.lower():
            log_info(self.log_path, "Content classified as interview, running diarization")
