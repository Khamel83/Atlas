...
from .utils import log_info, log_error, generate_markdown_summary, calculate_hash, convert_html_to_markdown
...
class InstapaperIngestor(BaseIngestor):
    ...
    def ingest_articles(self, limit: int = 0):
        if not all([self.login, self.password]):
            log_error(self.log_path, "Instapaper credentials not found in .env file. Skipping.")
            return
            ...
            try:
                ...
                while True:
                    ...
                    if not article_links:
                        ...
                        break
                    log_info(self.log_path, f"Found {len(article_links)} articles on the current page.")
                    for link_element in article_links:
                        if limit > 0 and processed_count >= limit:
                            log_info(self.log_path, f"Reached processing limit of {limit} articles.")
                            break
                        title = link_element.inner_text()
                        original_url = link_element.get_attribute('href')
                        article_container = page.locator(f'div.article_item:has(a[href="{original_url}"])')
                        text_view_url_path = article_container.locator('a.js_text_control').get_attribute('href')
                        text_view_url = f"https://www.instapaper.com{text_view_url_path}"
                        file_id = hashlib.sha1(original_url.encode('utf-8')).hexdigest()[:16]
                        ...
                        if os.path.exists(meta_path):
                            log_info(self.log_path, f"Article '{title}' ({file_id}) already processed. Skipping.")
                            continue
                        try:
                            log_info(self.log_path, f"Fetching content for '{title}' from {text_view_url}")
                            ...
                            log_info(self.log_path, f"Successfully processed and saved '{title}'.")
                            processed_count += 1
                            content_page.close()
                            sleep(1)
                        except PlaywrightTimeoutError:
                            log_error(self.log_path, f"Timeout loading content for '{title}'. Skipping.")
                        except Exception as e:
                            self.error_handler.handle_error(Exception(f"Error processing article '{title}': {e}"), self.log_path)
                    if limit > 0 and processed_count >= limit:
                        break
                    ...
                    if next_button.count() > 0:
                        ...
                        try:
                            next_button.click()
                            page.wait_for_url("https://www.instapaper.com/u/page/*", timeout=120000)
                        except PlaywrightTimeoutError:
                            log_error(self.log_path, "Timeout while navigating to the next page. Ending scrape.")
                            break
                    else:
                        log_info(self.log_path, "No 'Next' button found. Reached the end of articles.")
                        break
            except PlaywrightTimeoutError:
                log_error(self.log_path, "Timeout during login or navigation. Check credentials or network.")
                page.screenshot(path="instapaper_error.png")
                log_info(self.log_path, "Screenshot saved to instapaper_error.png")
            except Exception as e:
                self.error_handler.handle_error(Exception(f"An unexpected error occurred during the Instapaper scrape: {e}"), self.log_path)
            finally:
