...
class ModelUsageTracker:
    ...
    def record_usage(self, model: str, tokens_used: int = 0, cost: float = 0.0, success: bool = True):
        """Record model usage"""
        ...
        model_data["success_rate"] = (model_data["total_requests"] - model_data["failures"]) / model_data["total_requests"]
        ...
        if today not in self.usage_data["daily_totals"]:
            self.usage_data["daily_totals"][today] = {"requests": 0, "tokens": 0, "cost": 0.0}
        ...
        if month not in self.usage_data["monthly_totals"]:
            self.usage_data["monthly_totals"][month] = {"requests": 0, "tokens": 0, "cost": 0.0}
    ...
    def get_usage_summary(self) -> Dict:
        ...
        return {
            ...
            "active_models": len([m for m in self.usage_data["models"].values() if m.get("last_used")])
        }
...
class ModelDiscovery:
    ...
    def discover_free_models(self) -> List[str]:
        ...
        try:
            ...
            if not api_key:
                logging.warning("No OpenRouter API key found, using fallback model list")
            ...
            if response.status_code != 200:
                logging.warning(f"OpenRouter API returned {response.status_code}, using fallback")
                return self._get_fallback_free_models()
            ...
            logging.info(f"Discovered {len(free_models)} free models and {len(paid_models)} paid models")
            return free_models
...
class EnhancedModelSelector:
    ...
    def _build_tier_map(self) -> Dict:
        ...
        return {
            "premium": {
                "free": [
                    self.config.get("MODEL_FREE_PREMIUM_1", "deepseek/deepseek-r1:free"),
                    self.config.get("MODEL_FREE_PREMIUM_2", "deepseek/deepseek-v3:free"),
                    self.config.get("MODEL_FREE_PREMIUM_3", "meta-llama/llama-3.1-8b-instruct:free")
                ],
                "paid": self.config.get("MODEL_PREMIUM", "google/gemini-2.0-flash-lite-001")
            },
            "fallback": {
                "free": [
                    self.config.get("MODEL_FREE_FALLBACK_1", "google/gemma-2-9b-it:free"),
                    self.config.get("MODEL_FREE_FALLBACK_2", "mistralai/mistral-7b-instruct:free"),
                    self.config.get("MODEL_FREE_FALLBACK_3", "qwen/qwen-2.5-7b-instruct:free")
                ],
                "paid": self.config.get("MODEL_FALLBACK", "google/gemini-2.0-flash-lite-001")
            },
            "budget": {
                "free": [
                    self.config.get("MODEL_FREE_BUDGET_1", "mistralai/mistral-7b-instruct:free"),
                    self.config.get("MODEL_FREE_BUDGET_2", "qwen/qwen-2.5-7b-instruct:free"),
                    self.config.get("MODEL_FREE_BUDGET_3", "google/gemma-2-9b-it:free")
                ],
                "paid": self.config.get("MODEL_BUDGET", "mistralai/mistral-7b-instruct")
            }
        }
    
    def select_model(self, tier: str = "fallback", require_fast: bool = False, require_quality: bool = False) -> Tuple[str, str]:
        """
    ...
    def _is_model_usable(self, model: str) -> bool:
        ...
        if model_stats.get("success_rate", 1.0) < 0.7:  # Less than 70% success rate
            self.logger.debug(f"Model {model} has low success rate: {model_stats.get('success_rate')}")
            return False
        
        return True
    
    def record_model_usage(self, model: str, tokens_used: int = 0, cost: float = 0.0, success: bool = True):
    ...
    def get_model_recommendations(self) -> Dict:
        ...
        if total_requests > 0:
            ...
            recommendations["free_usage_percentage"] = (free_requests / total_requests) * 100
        ...
        recommendations["most_used_models"] = sorted(model_usage, key=lambda x: x[1], reverse=True)[:5]
...
def select_model(tier: str = "fallback", require_fast: bool = False, require_quality: bool = False) -> str:
    """
...
def record_model_usage(model: str, tokens_used: int = 0, cost: float = 0.0, success: bool = True):
    """Record model usage for tracking"""
