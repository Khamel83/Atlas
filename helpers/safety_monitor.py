...
class SafetyMonitor:
    ...
    def scan_content_for_sensitive_data(self, content: str, source: str) -> Dict[str, Any]:
        ...
        if findings:
            self.logger.warning(f"Sensitive data detected in {source}: {len(findings)} patterns found")
...
def check_pre_run_safety(config: Dict[str, Any]) -> bool:
    ...
    for env_file in env_files:
        if os.path.exists(env_file):
            ...
            if permissions not in ["600", "400"]:
                issues.append(f"⚠️  {env_file} file should have 600 permissions: chmod 600 {env_file}")
            break  # Only check the first one that exists
    ...
    if os.path.exists(data_dir):
        ...
        if permissions not in ["700", "755"]:
            issues.append(f"⚠️  Data directory should have 700 permissions: chmod 700 {data_dir}")
