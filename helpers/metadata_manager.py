...
class MetadataManager:
    ...
    def get_all_metadata(self, filters: Dict[str, Any] = None) -> List[ContentMetadata]:
        ...
        for metadata in all_metadata:
            ...
            try:
                metadata_date = datetime.fromisoformat(metadata.created_at.replace('Z', '+00:00'))
    ...
    def get_all_metadata_by_type(self, content_type: ContentType) -> List[ContentMetadata]:
        ...
        for metadata in all_metadata:
            try:
                ...
                if updated < cutoff:
                    ...
                    relevance_score = len(metadata.tags) * 0.1  # More tags = more relevant
    ...
    def get_tag_patterns(self, min_frequency: int = 2) -> Dict[str, Any]:
        ...
        filtered_tags = {tag: freq for tag, freq in tag_frequencies.items() if freq >= min_frequency}
        ...
        for tag, frequency in filtered_tags.items():
            patterns['tag_source_analysis'][tag] = {
                ...
                'average_usage': frequency / len(tag_sources[tag]) if tag_sources[tag] else 0
            }
        ...
        for tag, analysis in patterns['tag_source_analysis'].items():
            if analysis['frequency'] >= 3 and analysis['unique_sources'] >= 2:
                patterns['trending_tags'].append({
                    ...
                    'diversity_score': analysis['unique_sources'] / analysis['frequency']
                })
    ...
    def get_temporal_patterns(self, time_window: str = 'month') -> Dict[str, Any]:
        ...
        if len(sorted_periods) >= 2:
            ...
            older_periods = sorted_periods[:-3] if len(sorted_periods) > 3 else sorted_periods[:1]
            
            recent_avg = sum(content_volume[p] for p in recent_periods) / len(recent_periods)
            older_avg = sum(content_volume[p] for p in older_periods) / len(older_periods)
