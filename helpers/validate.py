...
class ConfigValidator:
    ...
    def validate_config(self, config: Dict) -> Tuple[List[ValidationError], List[ValidationError]]:
        """
    ...
    def _validate_model_configuration(self, config: Dict):
        ...
        if not config.get("llm_model_budget") and provider in ["openrouter", "deepseek"]:
            self.warnings.append(ValidationError(
    ...
    def _validate_security_settings(self, config: Dict):
        ...
        sensitive_fields = ["OPENROUTER_API_KEY", "DEEPSEEK_API_KEY", "YOUTUBE_API_KEY", 
                          "INSTAPAPER_PASSWORD", "NYT_PASSWORD"]
        
        for field in sensitive_fields:
            value = config.get(field)
            if value and ("test" in value.lower() or "example" in value.lower() or "your_" in value.lower()):
                self.warnings.append(ValidationError(
                    field=field,
    ...
    def _validate_performance_settings(self, config: Dict):
        ...
        if not config.get("llm_model_budget") and config.get("llm_provider") == "openrouter":
    ...
    def format_validation_report(self, errors: List[ValidationError], warnings: List[ValidationError]) -> str:
...
def validate_config_enhanced(config: Dict) -> Tuple[List[ValidationError], List[ValidationError]]:
    """
