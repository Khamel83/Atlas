...
class TranscriptLookup:
    ...
    def get_transcript(self, url: str, audio_path: Optional[str] = None, 
        ...
        for source in self.sources:
            try:
                transcript = self._try_source(source, url, audio_path, metadata, log_path)
                if transcript and self._is_valid_transcript(transcript):
                    log_info(log_path, f"Found transcript via {source.name} (quality: {source.quality_score})")
                    return transcript
    ...
    def _try_source(self, source: TranscriptSource, url: str, audio_path: Optional[str], 
                   metadata: Optional[Dict], log_path: str) -> Optional[str]:
    ...
    def _get_youtube_transcript(self, url: str, log_path: str) -> Optional[str]:
        ...
        try:
            ...
            if transcript_text:
                log_info(log_path, f"Found YouTube transcript ({len(transcript_text)} chars)")
    ...
    def _get_podcast_shownotes_transcript(self, url: str, metadata: Optional[Dict], 
                                        log_path: str) -> Optional[str]:
        ...
        try:
            ...
            if transcript_text:
                log_info(log_path, f"Found transcript in show notes ({len(transcript_text)} chars)")
                return transcript_text
    ...
    def _get_rss_transcript(self, url: str, metadata: Optional[Dict], log_path: str) -> Optional[str]:
        ...
        try:
            ...
            if not any(indicator in description.lower() for indicator in self.transcript_indicators):
                return None
            ...
            if transcript:
                log_info(log_path, f"Found transcript in RSS description ({len(transcript)} chars)")
                return transcript
    ...
    def _get_rev_transcript(self, url: str, log_path: str) -> Optional[str]:
        ...
        try:
            ...
            if transcript_div:
                ...
                log_info(log_path, f"Found Rev.com transcript ({len(transcript)} chars)")
                return transcript
    ...
    def _get_otter_transcript(self, url: str, log_path: str) -> Optional[str]:
        ...
        try:
            ...
            if transcript_content:
                ...
                log_info(log_path, f"Found Otter.ai transcript ({len(transcript)} chars)")
                return transcript
    ...
    def _get_cached_transcript(self, url: str, log_path: str) -> Optional[str]:
        ...
        try:
            ...
            cache_dir = Path(self.config.get("data_directory", "output")) / "transcript_cache"
    ...
    def _get_openrouter_transcript(self, audio_path: str, log_path: str) -> Optional[str]:
        ...
        try:
            ...
            if transcript and "failed" not in transcript.lower():
                log_info(log_path, f"OpenRouter transcription successful ({len(transcript)} chars)")
                return transcript
    ...
    def _get_local_transcript(self, audio_path: str, log_path: str) -> Optional[str]:
        ...
        try:
            ...
            if transcript:
                log_info(log_path, f"Local transcription successful ({len(transcript)} chars)")
                return transcript
    ...
    def _looks_like_transcript(self, text: str) -> bool:
        ...
        indicators_found = sum(1 for indicator in self.transcript_indicators 
        ...
        speaker_matches = sum(1 for pattern in speaker_patterns 
...
def get_transcript_with_lookup(url: str, audio_path: Optional[str] = None, 
