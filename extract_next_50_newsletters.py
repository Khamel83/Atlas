#!/usr/bin/env python3
"""
Extract next 50 accessible private newsletters (51-100)
"""
...
def extract_next_50_newsletters():
    """Extract newsletters 51-100"""
    
    print("⚡ EXTRACTING NEWSLETTERS 51-100")
    print("=" * 50)
    ...
    # Process newsletters 51-100
    newsletters_list = list(all_private.values())[50:100]
    print(f"\n🔄 Processing newsletters 51-100 ({len(newsletters_list)} newsletters)...")
    
    successful = 0
    extracted_content = {}
    
    for i, newsletter in enumerate(newsletters_list, 51):
        try:
    ...
    if extracted_content:
        ...
        for bookmark_id, content_data in extracted_content.items():
            try:
                import hashlib
                uid_source = f"private_51_100_{bookmark_id}_{content_data['title']}"
    ...
    report_file = f"extract_51_100_newsletters_report_{timestamp}.json"
    ...
    if successful > 0:
        ...
        print(f"📈 Total so far: {50 + successful} newsletters extracted")
...
<body>
    ...
    <div class="instapaper-info">
        <p><strong>Source:</strong> Private Newsletter (Batch 51-100)</p>
...
"""
    ...
    metadata = {
        "uid": atlas_uid,
        "content_type": "private_newsletter_batch_51_100",
        ...
        "fetch_method": "instapaper_api_batch_51_100_extraction",
        "fetch_details": {
            "bookmark_id": bookmark_id,
            "content_length": len(full_text),
            "successful": True,
            "extraction_batch": "batch_51_100"
        },
        "type_specific": {
            "bookmark_id": bookmark_id,
